...
function addRow(ticket, data, numDistinctValues, numChanges, metadataMap) {
    ...
    $(tr).append($('<td>').addClass('currency').html(
        formatPercentWithSign(100 * (end - start) / start)).addClass(diffClass).attr('data-value', 100 * (end - start) / start));

    $(tr).append($('<td>').addClass('bool').html(formatBool(isLastMinute)));
...
function doLoad(metadataCsvFile, csvFile, inventoryCsvFile) {
    ...
    console.log('loading metadata from ' + metadataCsvFile + '...');
    // Map tickets to metadata objects.
    let row = (d) => {
        if (d.ticket.startsWith('seatgeek')) {
            d.url = d.url.replace(
                'https://www.seatgeek.com/',
                // todo
                'https://seatgeek.com/super-bowl-lvi-cincinnati-bengals-vs-los-angeles-rams-tickets/2-13-2022-inglewood-california-sofi-stadium/nfl/r/5428684/');
        }
        return {
            ticket: d.ticket,
            row: d.row,
            ...
            min: d.min,
            max: d.max,
            distinctPricesCount: d.distinctPricesCount,
            pricesCount: d.pricesCount,
        };
    }
...
HistogramController.prototype.setDate = function (date) {
    this.date_ = date;
    this.render();
};
...
HistogramController.prototype.tick = function () {
    ...
    let dates = $('#scatter .slider').val(),
        index = dates.indexOf(this.date_),
        max = dates.length;
    index++;
    if (index >= max) {
    ...
    this.date_ = dates[index];
    this.render(function () {
        setTimeout(this.tick.bind(this), this.tickMillis_);
    }.bind(this));
};
...
HistogramController.prototype.render = function (opt_done) {
    ...
    redrawHistograms(this.date_, opt_done);
    let dates = $('#scatter .slider').val();
    let index = dates.indexOf(String(this.date_));
    this.skipThisChange_ = true;
    $("#scatter .slider").slider('option', 'value', index)
};
...
function loadDataScatter(metadataMap, data) {
    ...
    let datesSet = {};
    data.forEach(d => {
        datesSet[d.date] = true;
    });
    let dates = Object.keys(datesSet).map(d => new Date(Date.parse(d)));
    dates.sort((a, b) => a - b);

    let c = new HistogramController();

    $("#scatter .slider").slider({
        change: function (event, ui) {
            if (c.skipThisChange_) {
                c.skipThisChange_ = false;
                return;
            }
            let date = $("#scatter .slider").val()[ui.value];
            c.reset();
            c.setDate(date);
        },
        min: 0,
    ...
    $("#scatter .slider").val(dates);
    $('#scatter .date .max .val').text(String(maxDate));
    $('#scatter .date .min .val').text(String(minDate));
...
function redrawHistograms(date, opt_done) {
    $('#scatter .date .cur .val').text(date);

    let data = scatterData.filter(d => String(d.date) == String(date));
    ...
    let margin = {
        ...
        width = 1400 / 3 - margin.left - margin.right,
        height = 600 - margin.top - margin.bottom;

    let minPrice = d3.min(data, d => d.price),
        maxPrice = d3.max(data, d => d.price),
        x = d3.scaleLinear().domain([minPrice, 20000]).range([0, width]),
        histogram = d3.histogram().value(d => d.price).domain(x.domain()).thresholds(x.ticks(100)),
        tag = (d) => d.ticket.split('-')[0],
        bins1 = histogram(data.filter(d => tag(d) === 'stubhub')),
        bins2 = histogram(data.filter(d => tag(d) === 'seatgeek')),
        bins3 = histogram(data.filter(d => tag(d) === 'vividseats')),
        maxYx = Math.max(
            d3.max(bins1, d => d.length),
            d3.max(bins2, d => d.length),
    ...
    function drawTag(tag) {
        $('#scatter .histograms .' + tag).empty();

        let svg = d3.select('#scatter .histograms .' + tag)
            .append('svg')
            .attr('width', width + margin.left + margin.right)
            .attr('height', height + margin.top + margin.bottom)
            .append('g')
        ...
        switch (tag) {
            case 'stubhub':
                draw(bins1, 'rect1', '#69b3a2');
                svg.append('circle').attr('cx', 300).attr('cy', 30).attr('r', 6).style('fill', '#69b3a2');
                svg.append('text').attr('x', 320).attr('y', 30).text('stubhub').style('font-size', '15px').attr('alignment-baseline', 'middle');
                break;
            case 'seatgeek':
                draw(bins2, 'rect2', '#404080');
                svg.append('circle').attr('cx', 300).attr('cy', 30).attr('r', 6).style('fill', '#404080');
                svg.append('text').attr('x', 320).attr('y', 30).text('seatgeek').style('font-size', '15px').attr('alignment-baseline', 'middle');
                break;
            case 'vividseats':
                draw(bins3, 'rect3', '#808040');
                svg.append('circle').attr('cx', 300).attr('cy', 30).attr('r', 6).style('fill', '#808040');
                svg.append('text').attr('x', 320).attr('y', 30).text('vividseats').style('font-size', '15px').attr('alignment-baseline', 'middle');
                break;
            default:
...
function loadData(csvFile, metadataMap, inventoryData, done) {
    ...
    d3.csv(csvFile, row, function (error, data) {
        if (error) throw error;
