let rowFilter, sectionFilter, lastMinuteFilter, lastLastMinuteFilter;
...
function load() {
    ...
    lastLastMinuteFilter = params.get('lastLastMinute') != null;

    if (rowFilter || sectionFilter || lastMinuteFilter || lastLastMinuteFilter) {
        ...
        if (lastLastMinuteFilter) {
            filtersContainer
                ...
                .append(
                    ...
                    .addClass('badge-info')
                    .addClass('badge')
                    .text('LAST LAST minute changes')
                    .click(function (e) {
                        lastLastMinuteFilter = false;
                        reload();
                    })
                );
        }
    ...
    d3.csv('knicks_kings_metadata.csv', row, function (error, data) {
...
function loadData(metadataMap) {
    ...
    d3.csv('knicks_kings.csv', row, function (error, dataAll) {
        if (error) throw error;
        ...
        if (rowFilter || sectionFilter || lastMinuteFilter || lastLastMinuteFilter) {
            ...
            const lastLastMinuteThreshold = new Date(1643000409236); // 12am
            let filteredKeySet = [];
            dataAll.forEach((d) => {
                ...
                if (lastMinuteFilter) {
                    if (d.date < lastMinuteThreshold) {
                        return
                    }
                }
                if (lastLastMinuteFilter) {
                    if (d.date < lastLastMinuteThreshold) {
                        return
                    }
                }
        ...
        let sortCol = 13,
            sortDir = 'desc'; // # Moves
        if (lastMinuteFilter || lastLastMinuteFilter) {
            sortCol = 11; // End-Start%
            sortDir = 'asc';
        }
